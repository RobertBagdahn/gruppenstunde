# idea-creation-flow Specification
...
Frontend-Erstellungsfluss für Ideas (Typ `idea` und `knowledge`) auf `/create/idea` und `/create/knowledge`. Bietet zwei gleichwertige Wege: KI-gestützt (Refurbish) und manuell. Der KI-Weg ist abbrechbar, zeigt geschätzte Wartezeiten und bietet Fallback-Optionen bei Fehlern.

## Context

- **Frontend**: `NewIdeaPage.tsx` — 3-Schritt-Wizard (Step 0: Beschreiben, Step 1: Bearbeiten, Step 2: Vorschau & Speichern)
- **Backend**: `POST /api/ai/refurbish/` (Textaufbereitung), `POST /api/ai/generate-image/` (manuelle Bildgenerierung)
- **Abhängigkeiten**: `ai-features` Spec (Refurbish-Endpunkt), `error-handling` Spec (KI-Fehlercodes)
...
| Datei | Relevanz |
|-------|----------|
| `frontend/src/pages/NewIdeaPage.tsx` | Haupt-Wizard mit Zwei-Wege-Auswahl, AbortController, Fehler-UI |
| `frontend/src/api/ai.ts` | `useRefurbish` und `useGenerateImage` Hooks mit signal/abort Support |
| `frontend/src/schemas/idea.ts` | `AiRefurbishSchema`, `AiErrorSchema` |
