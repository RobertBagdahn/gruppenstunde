...
`Recipe` SHALL ein denormalisiertes Feld `cached_energy_total_kj` (Float, nullable)
führen, das die Gesamtenergie des Rezepts in kJ über alle `servings` enthält.
...
`cached_energy_total_kj = energy_kj_per_100g * (total_weight_g / 100)`.
...
- **THEN** `cached_energy_total_kj` entspricht der Summe der Zutaten-Energien
...
- **THEN** `cached_energy_total_kj` ist `None` oder `0`, ohne Fehler
...
- **THEN** `cached_energy_total_kj` wird durch das bestehende Signal neu berechnet

### Requirement: Realistische Energie im Meal-Item Output

`MealItemOut.energy_kj` SHALL die tatsächliche Gesamtenergie des Rezept-Items in kJ
...
pro-100g-Wert `cached_energy_kj` basieren.
...
- **THEN** `energy_kj` entspricht `cached_energy_total_kj * factor * norm_portions / servings`
  und ergibt nach Umrechnung (`/ norm_portions / 4.184`) eine realistische
  Pro-Portion-Kalorienzahl

#### Scenario: Rezept ohne gecachte Gesamtenergie
- **WHEN** das referenzierte Rezept `cached_energy_total_kj = None` hat
- **THEN** `energy_kj` ist `None`

### Requirement: Realistische Gesamtenergie im Meal Output

`MealOut.total_energy_kj` SHALL die Summe der `energy_kj`-Werte aller Items der
Mahlzeit liefern, berechnet auf Basis von `cached_energy_total_kj`. Der Wert MUST
...
- **THEN** `total_energy_kj` ist die Summe der Item-`energy_kj`-Werte

#### Scenario: Konsistenz mit Nutrition-Summary
- **WHEN** dieselbe Mahlzeit über `MealOut.total_energy_kj` und über den
...
korrigierten Energiewerten beruhen, sodass der Ist-Prozentwert die tatsächliche
