# recipe-ingredient-requirement Specification

## Purpose

Specifies the validation rule that recipes must have at least one RecipeItem (ingredient) before they can transition from draft to submitted or approved status. This ensures data quality — recipes without ingredients have no nutritional values, no price, and cannot be used meaningfully in meal plans.
