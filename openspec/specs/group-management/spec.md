...
Hierarchisches Gruppenverwaltungssystem fuer Pfadfinder-Organisationen. Unterstuetzt verschachtelte Gruppen (z.B. Stamm -> Sippe), Mitgliedschaft mit Rollen (Admin/Mitglied), drei Beitrittsmechanismen (offen, Beitrittscode, Beitrittsanfrage) und Mitgliedschaftsvererbung durch die Gruppenhierarchie. Gruppen sind soft-deletable.

## Requirements

### Requirement: Gruppen CRUD

Das System MUST vollstaendige CRUD-Operationen fuer UserGroups ueber `/api/groups/` bereitstellen. Gruppen werden per Slug identifiziert.
...
- AND der Ersteller wird als Admin-Mitglied hinzugefuegt
...
#### Scenario: Gruppe soft-loeschen

- GIVEN ein Gruppen-Admin
- WHEN der Admin die Gruppe loescht
...
Das System SHALL Eltern-Kind-Gruppenbeziehungen ueber das `parent`-Feld (FK zu sich selbst, nullable) fuer die Organisationshierarchie unterstuetzen.
...
- AND die Gruppendetail-Antwort enthaelt `inherited_member_count`
...
- THEN enthaelt die Antwort `ancestors` als geordnete Liste aller Vorfahren
...
#### Scenario: Mitglied hinzufuegen

- GIVEN ein Gruppen-Admin
- WHEN der Admin einen Benutzer zur Gruppe hinzufuegt
...
#### Scenario: Zum Admin befoerdern

- GIVEN ein Gruppen-Admin und ein bestehendes Mitglied
- WHEN der Admin das Mitglied befoerdert
- THEN aendert sich die Rolle des Mitglieds auf "admin"
- AND das Mitglied erhaelt Gruppen-Verwaltungsberechtigungen

#### Scenario: Mitglied entfernen (Gruppe verlassen)

- GIVEN ein Gruppen-Mitglied
- WHEN das Mitglied die Gruppe per DELETE `/api/groups/{slug}/members/{membershipId}/` verlaesst
- THEN wird die GroupMembership geloescht
- AND der Benutzer verliert Zugriff auf gruppenspezifische Funktionen

### Requirement: Beitrittsmodus

Das System SHALL den Beitrittsmodus einer Gruppe ueber ein explizites Feld `join_mode` steuern. Derzeit im Code als Kombination aus `free_to_join` (BooleanField) und `join_code` (CharField) implementiert — Migration zu `join_mode` ist geplant.
...
- THEN wird der Benutzer sofort als Mitglied hinzugefuegt (Response: 200 mit GroupMember)
...
- AND bei gueltigem Code wird der Benutzer als Mitglied hinzugefuegt
...
- THEN wird der Benutzer als Mitglied hinzugefuegt
- AND der Anfrage-Status aendert sich auf "approved"

#### Scenario: Beitrittsanfrage ablehnen

- GIVEN eine ausstehende GroupJoinRequest
- WHEN ein Gruppen-Admin die Anfrage ablehnt
- THEN aendert sich der Anfrage-Status auf "rejected"
- AND der Benutzer wird nicht zur Gruppe hinzugefuegt
...
- THEN werden alle Gruppen zurueckgegeben, denen der Benutzer angehoert
...
- THEN werden alle JoinRequests des Benutzers zurueckgegeben

#### Scenario: Gruppen suchen

- GIVEN oeffentliche Gruppen im System
- WHEN ein Benutzer GET `/api/groups/?q=adler` aufruft
- THEN werden passende Gruppen basierend auf dem Suchbegriff zurueckgegeben

#### Scenario: Gruppendetail mit Mitgliedern

- GIVEN eine Gruppe mit Mitgliedern
- WHEN ein Benutzer GET `/api/groups/{slug}/` aufruft
- THEN werden die Gruppendetails mit `members`, `children`, `ancestors`, `inherited_member_count` und `join_code` (nur fuer Admins sichtbar) zurueckgegeben
...
- Derzeit wird der Beitrittsmodus ueber die Kombination `free_to_join` + `join_code` bestimmt.
