...
Verhindert unerwünschtes automatisches Zoomen auf iOS Safari bei Seitenaufrufen in beiden Frontends (main + food). Definiert die erforderliche Viewport-Konfiguration und horizontale Overflow-Prävention.

## Context

- **Betrifft**: `frontend/` und `frontend-food/` (beide React SPAs)
- **Problem**: iOS Safari zoomt bei jedem Seitenaufruf leicht ein, wenn `maximum-scale` im Viewport-Meta fehlt oder Elemente horizontal überlaufen
