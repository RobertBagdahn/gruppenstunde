...
- **THEN** on success the system SHALL navigate to `/ingredients/<slug>`
...
- **THEN** the system SHALL navigate to `/ingredients/new`
- **THEN** no toast error SHALL be shown
- **THEN** the current ingredient name from the input SHALL NOT be pre-filled (stateless navigation)
