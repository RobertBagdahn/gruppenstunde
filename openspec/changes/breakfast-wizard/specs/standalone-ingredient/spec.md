...
### Requirement: standalone_type als Rezept-Typ-Bestimmung
**Reason**: `standalone_type` diente ausschließlich dazu, den `recipe_type` des automatisch erzeugten Dummy-Rezepts zu setzen. Da Dummy-Rezepte entfallen, ist das Feld ohne Funktion.
**Migration**: `standalone_type`-Feld per `makemigrations supply` aus dem Datenbankschema entfernen. Pydantic-Schemas (`supply/schemas/ingredients.py`) und Zod-Schemas im Frontend bereinigen.
