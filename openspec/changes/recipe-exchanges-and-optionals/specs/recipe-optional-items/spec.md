...
Eine Zutat MUST entweder `is_optional=True` ODER Teil einer Exchange-Gruppe sein — nie beides. DB-seitig via CHECK CONSTRAINT erzwungen.
...
#### Scenario: Optional-Flag entfernen

- **WHEN** der Autor den "Optional"-Toggle deaktiviert
- **THEN** wird `RecipeItem.is_optional = False` gesetzt; bestehende MealItemSplits für diese Zutat bleiben unverändert
