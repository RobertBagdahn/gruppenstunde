...
### Requirement: Eigenständige Rezept-Import-Seite
**Reason**: Die Route `/recipes/import` und die Komponente `RecipeImportPage.tsx` sind von keiner Stelle der Anwendung verlinkt und stellen einen vierten, verwaisten Erstellungsweg dar. Der URL-Import ist vollständig im Smart-Eingabefeld aufgegangen.

**Migration**: Nutzer verwenden `/recipes/new` und fügen die Rezept-URL in das Smart-Eingabefeld ein. Die Route `/recipes/import` entfällt ersatzlos.
