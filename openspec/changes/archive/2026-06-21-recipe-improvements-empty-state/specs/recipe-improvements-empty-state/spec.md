...
Die vier Render-Zweige in Reihenfolge:

1. `isLoading` → Skeleton-Pulse
2. `error` → Fehlerkarte mit Text „Verbesserungsvorschläge konnten nicht geladen werden." und Retry-Button
3. `data.all_good` → Grüne Erfolgskarte
4. `!data || data.items.length === 0` → Neutrale Info-Karte mit `data.message`
...
- **THEN** enthält sie ein Info-Icon (`info` aus Lucide) links vom Text
