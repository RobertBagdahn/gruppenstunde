...
- [x] 5.2 PackingList: Update suggestion response with `ai_interaction_id`, update `packinglist/services/suggestion_service.py`
...
- [x] 8.1 Add `useAiInteractions()` hook — paginated list with filter params (`frontend/src/api/aiInteraction.ts`)
- [x] 8.2 Add `useAiInteractionDetail()` hook — single interaction detail
- [x] 8.3 Add `useAiUserCosts()` hook — user cost aggregation
- [x] 8.4 Update existing AI hooks to parse `ai_interaction_id` from responses (`frontend/src/api/ai.ts`, `frontend-food/src/api/ai.ts`)

## 9. Frontend — Admin Pages

- [x] 9.1 Create `KiLogPage` — paginated table with filter bar, expand for prompt/response (`frontend/src/pages/admin/KiLogPage.tsx`)
- [x] 9.2 Create `KiKostenPage` — per-user cost table sorted by cost descending (`frontend/src/pages/admin/KiKostenPage.tsx`)
- [x] 9.3 Update existing admin stats page to show cost data (`frontend/src/pages/admin/` + useAiInteractionStats hook)
- [x] 9.4 Add routes for new admin pages in `frontend/src/App.tsx`
- [x] 9.5 Add admin navigation entries for KI-Log and KI-Kosten

## 10. Frontend — Vote Buttons Integration

- [x] 10.1 Add `AiVoteButtons` to `ContentStepper` (refurbish, improve_text result areas)
- [x] 10.2 Add `AiVoteButtons` to `InlineEditor` (improve_text result)
- [x] 10.3 Add `AiVoteButtons` to `TitleImageEditor` (generate_image result)
- [x] 10.4 Add `AiVoteButtons` to `AiCreateDialog` / AI suggest supply dialogs
- [x] 10.5 Add `AiVoteButtons` to `StepInvitationText` (event invitation result)
- [x] 10.6 Add `AiVoteButtons` to `PackingListDetailPage` (packing list AI suggestions)
- [x] 10.7 In `frontend-food`: Add `AiVoteButtons` to recipe AI wizard steps and ingredient creation stepper
- [x] 10.8 In `frontend-food`: Add `AiVoteButtons` to meal plan AI suggest dialog

## 11. Verification

- [x] 11.1 Run backend tests: `uv run python manage.py test`
- [x] 11.2 Run backend lint: `uv run ruff check backend/`
- [x] 11.3 Run frontend typecheck: `npm run typecheck` (in frontend/)
- [x] 11.4 Run frontend-food typecheck: `npm run typecheck` (in frontend-food/)
- [x] 11.5 Manual smoke test: trigger AI calls, verify tokens and costs in admin log, verify vote buttons appear and work
