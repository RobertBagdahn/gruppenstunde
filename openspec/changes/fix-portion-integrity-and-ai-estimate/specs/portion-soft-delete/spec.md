...
### Requirement: RecipeItem mit gelöschter Portion (verwaiste Referenz erlaubt)
**Reason**: Verwaiste Referenzen auf soft-gelöschte Portionen führten nachweislich zu falschen Gramm-Berechnungen im Edit-Mode und zu Datenkorruption bei der AI-Mengenschätzung (siehe `portion-integrity-guardrails`). Das Verhalten "Referenz bleibt unverändert bestehen" wird durch automatisches Rebind ersetzt.
**Migration**: Bestehende RecipeItems mit toter Portion-Referenz werden durch den `recipe-quantity-repair`-Command einmalig umgehängt (siehe `recipe-quantity-repair`-Capability).
