...
export default function ShoppingListItemRow({
  ...
  const formatItemQuantity = (g: number, unit: string): string => {
    if (g === 0) return '';
    if (unit === 'g' || unit === 'kg') {
      return formatWeight(g).display;
    }
    return g > 0 ? `${g} ${unit}` : '';
  };
          ...
          <div className="flex items-center gap-2 flex-wrap">
            ...
            {item.quantity_g > 0 && (
              <span className="text-xs text-muted-foreground">
                {formatItemQuantity(item.quantity_g, item.unit)}
              </span>
            )}
          </div>
          {item.note && (
        ...
        <div className="pl-16 pr-2 pb-2 space-y-1">
          {item.sources.map((source, idx) => (
            <div key={idx} className="flex items-center justify-between text-xs text-muted-foreground">
              ...
              <span>{formatWeight(source.quantity_g).display}</span>
            </div>
          ))}
        </div>
      )}
