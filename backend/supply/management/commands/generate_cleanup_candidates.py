...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            "--output", type=str, default="cleanup_candidates.txt", help="Output file for candidate list"
        )
        parser.add_argument("--batch-size", type=int, default=60, help="Number of ingredients per AI request")
...
{items_text}"""
        ...
        try:
            ...
            self.stdout.write(self.style.ERROR(f"AI Error: {e!s}"))

        # Safe default: keep all
        return [
            {"pk": item["pk"], "name": item["name"], "description": item["description"], "suitable": True}
    ...
    def handle(self, *args, **options):
        ...
        for batch_start in range(0, len(ingredients), batch_size):
            ...
            self.stdout.write(f"[{batch_start+1}/{len(ingredients)}] Batch {batch_start+1}-{batch_end}...")

            batch_data = [{"pk": ing["pk"], "name": ing["name"], "description": ing["description"]} for ing in batch]
        ...
        self.stdout.write(self.style.SUCCESS(f"✓ {len(candidates_to_remove)} candidates saved to {output_file}"))
        self.stdout.write(self.style.WARNING("→ Edit the file and delete lines for ingredients you want to KEEP"))
        self.stdout.write(f"→ Then run: uv run python manage.py execute_cleanup --file {output_file}")
