...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument("--file", type=str, required=True, help="Path to candidate file")
    ...
    def handle(self, *args, **options):
        ...
        if not os.path.exists(candidate_file):
            self.stdout.write(self.style.ERROR(f"File not found: {candidate_file}"))
            return
        ...
        self.stdout.write(self.style.SUCCESS(f"✓ Ingredients: {initial_count} → {final_count}"))
