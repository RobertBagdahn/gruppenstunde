...
class Command(BaseCommand):
    ...
    def calculate_pair_similarities(self, pairs, embeddings):
        ...
        for pair in pairs:
            ...
            if ing_a_id in embeddings and ing_b_id in embeddings:
                cosine_sim = self.cosine_similarity(embeddings[ing_a_id], embeddings[ing_b_id])
    ...
    def handle(self, *args, **options):
        ...
        if r_squared > 0.90 and accuracy > 90:
            self.stdout.write(
                self.style.SUCCESS(f"✓ Fit quality is excellent (R²={r_squared:.3f}, accuracy={accuracy:.1f}%)")
            )
            ...
            self.stdout.write(self.style.ERROR(f"✗ Fit quality is poor (R²={r_squared:.3f}, accuracy={accuracy:.1f}%)"))
