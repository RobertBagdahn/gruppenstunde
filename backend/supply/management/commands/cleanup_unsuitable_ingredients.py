...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            "--batch-size", type=int, default=40, help="Number of ingredients to classify per AI request"
        )
...
{items_text}"""
        ...
        try:
            ...
            if response:
                ...
                while len(results) < len(batch_items):
                    idx = len(results)
                    results.append({"pk": batch_items[idx]["pk"], "name": batch_items[idx]["name"], "suitable": True})

                return results
            else:
                # No response - keep all (safe)
                return [{"pk": item["pk"], "name": item["name"], "suitable": True} for item in batch_items]
    ...
    def handle(self, *args, **options):
        ...
        for batch_start in range(0, len(ingredients), batch_size):
            ...
            for result in results:
                if not result["suitable"]:
                    ...
                    if ing:
                        self.stdout.write(
                            f"  REMOVE: pk {result['pk']:5d} - {ing['name'][:45]:<45}", self.style.WARNING
                        )

        # Summary
        self.stdout.write(self.style.SUCCESS("\n✓ AI classified all ingredients"))
        self.stdout.write(f"Marked for removal: {len(unsuitable_pks)} ingredients")

        if dry_run:
            self.stdout.write(self.style.WARNING("DRY RUN: No changes made to fixture"))
            return
        ...
        self.stdout.write(
            self.style.SUCCESS(f"✓ Ingredients: {initial_count} → {final_count} (removed {len(unsuitable_pks)})")
        )
        self.stdout.write(self.style.SUCCESS(f"✓ Updated: {fixture_path}"))
