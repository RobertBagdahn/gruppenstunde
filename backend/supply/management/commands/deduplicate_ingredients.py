...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            "--output", type=str, default="ingredient_mapping.json", help="Output file for mapping (old_pk → new_pk)"
        )
    ...
    def handle(self, *args, **options):
        ...
        for base, variants in grouped.items():
            ...
            for variant in variants:
                if variant["pk"] != canonical_pk:
                    ...
                    merge_count += 1
                    self.stdout.write(f"    → Remove: pk {old_pk}: {variant['fields']['name']}", self.style.WARNING)
        ...
        if not dry_run:
            ...
            self.stdout.write(self.style.SUCCESS(f"✓ Removed {len(pks_to_remove)} duplicate ingredients"))
            self.stdout.write(self.style.SUCCESS(f"✓ Updated {updated_portions_in_fixture} portion references"))
