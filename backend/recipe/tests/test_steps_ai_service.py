...
class TestGenerateStepsFromItems(TestCase):
    ...
    def setUp(self):
        ...
        self.water_portion = Portion.objects.create(ingredient=self.water, measuring_unit=self.unit, quantity=50)

        self.flour_item = RecipeItem.objects.create(recipe=self.recipe, portion=self.flour_portion, quantity=2)
        self.water_item = RecipeItem.objects.create(recipe=self.recipe, portion=self.water_portion, quantity=1)
    ...
    def test_generate_steps_from_items_no_ingredients_returns_empty(self, mock_gemini):
        ...
        recipe_empty = Recipe.objects.create(title="Empty Recipe", slug="empty-recipe", created_by=self.user)

        # Should return empty immediately without calling Gemini
        steps = AiStepService.generate_steps_from_items(recipe=recipe_empty, user=self.user)
...
class TestSuggestIngredientAssignment(TestCase):
    ...
    def setUp(self):
        ...
        RecipeItem.objects.create(recipe=self.recipe, portion=self.flour_portion, quantity=2)
        RecipeItem.objects.create(recipe=self.recipe, portion=self.water_portion, quantity=1)
    ...
    def test_suggest_ingredients_success(self, mock_gemini):
        ...
        suggestions, _interaction_id = AiStepService.suggest_ingredient_assignment(
            step_instruction="Mix flour with water", recipe=self.recipe, user=self.user
        )
...
class TestImproveStepInstruction(TestCase):
    ...
    def test_improve_step_instruction_precise_tone(self, mock_gemini):
        ...
        improved, _interaction_id = AiStepService.improve_step_instruction(
            instruction="Add flour and water, then mix it all together", tone="präzise", user=self.user
        )
    ...
    def test_improve_step_instruction_ausführlich_tone(self, mock_gemini):
        ...
        improved, _interaction_id = AiStepService.improve_step_instruction(
            instruction="Mix flour and water", tone="ausführlich", user=self.user
        )
    ...
    def test_improve_step_instruction_empty_returns_original(self):
        ...
        result, _interaction_id = AiStepService.improve_step_instruction(instruction="", tone="präzise", user=self.user)
        assert result == ""
...
class TestConvertMarkdownToSteps(TestCase):
    ...
    def test_convert_markdown_success(self, mock_gemini):
        ...
        steps = AiStepService.convert_markdown_to_steps(recipe=self.recipe, description=markdown, user=self.user)
    ...
    def test_convert_markdown_empty_recipe_items_returns_empty(self):
        ...
        recipe_empty = Recipe.objects.create(title="Empty Recipe", slug="empty-recipe-2", created_by=self.user)
