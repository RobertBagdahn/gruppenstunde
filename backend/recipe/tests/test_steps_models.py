...
from recipe.schemas import RecipeStepIn, RecipeStepIngredientIn
...
class TestRecipeStepModel(TestCase):
    ...
    def setUp(self):
        ...
        self.recipe = Recipe.objects.create(title="Test Recipe", slug="test-recipe", created_by=self.user)

    def test_create_recipe_step(self):
        """Test creating a recipe step."""
        step = RecipeStep.objects.create(
            recipe=self.recipe, instruction="Mix ingredients", sort_order=1, duration_minutes=5, section="Mixing"
        )
    ...
    def test_unique_sort_order_per_recipe(self):
        ...
        RecipeStep.objects.create(recipe=self.recipe, instruction="Step 1", sort_order=1)

        # Creating another step with same sort_order should fail
        with pytest.raises(Exception):  # IntegrityError
            RecipeStep.objects.create(recipe=self.recipe, instruction="Step 2", sort_order=1)

    def test_recipe_step_default_section(self):
        """Test that section defaults to empty string."""
        step = RecipeStep.objects.create(recipe=self.recipe, instruction="Test", sort_order=1)
        assert step.section == ""

    def test_recipe_step_null_duration(self):
        """Test that duration_minutes can be null."""
        step = RecipeStep.objects.create(recipe=self.recipe, instruction="Test", sort_order=1, duration_minutes=None)
...
class TestRecipeStepIngredientModel(TestCase):
    ...
    def setUp(self):
        ...
        self.portion = Portion.objects.create(ingredient=self.ingredient, measuring_unit=self.unit, quantity=100)

        # Create recipe item
        self.recipe_item = RecipeItem.objects.create(recipe=self.recipe, portion=self.portion, quantity=2)

        # Create step
        self.step = RecipeStep.objects.create(recipe=self.recipe, instruction="Mix", sort_order=1)

    def test_create_step_ingredient(self):
        """Test creating a step ingredient."""
        step_ing = RecipeStepIngredient.objects.create(
            step=self.step, recipe_item=self.recipe_item, quantity_modifier=1.5, preparation="sifted", sort_order=1
        )
    ...
    def test_unique_step_recipe_item_constraint(self):
        ...
        RecipeStepIngredient.objects.create(
            step=self.step, recipe_item=self.recipe_item, quantity_modifier=1.0, sort_order=1
        )

        # Creating duplicate should fail
        with pytest.raises(Exception):  # IntegrityError
            RecipeStepIngredient.objects.create(
                step=self.step, recipe_item=self.recipe_item, quantity_modifier=0.5, sort_order=2
            )

    def test_step_ingredient_default_quantity_modifier(self):
        """Test that quantity_modifier defaults to 1.0."""
        step_ing = RecipeStepIngredient.objects.create(step=self.step, recipe_item=self.recipe_item, sort_order=1)
        assert step_ing.quantity_modifier == 1.0

    def test_step_ingredient_empty_preparation(self):
        """Test that preparation can be empty."""
        step_ing = RecipeStepIngredient.objects.create(
            step=self.step, recipe_item=self.recipe_item, preparation="", sort_order=1
        )
...
class TestRecipeStepSchemas(TestCase):
    ...
    def test_recipe_step_in_schema_valid(self):
        ...
        data = RecipeStepIn(
            sort_order=1, instruction="Mix flour and water", duration_minutes=10, section="Mixing", step_ingredients=[]
        )
    ...
    def test_recipe_step_ingredient_in_schema(self):
        ...
        data = RecipeStepIngredientIn(recipe_item_id=1, quantity_modifier=1.5, preparation="diced", sort_order=1)
...
class TestBatchUpdateEndpoint(TestCase):
    ...
    def setUp(self):
        ...
        # Create recipe items
        self.item1 = RecipeItem.objects.create(recipe=self.recipe, portion=self.portion, quantity=1)
        self.item2 = RecipeItem.objects.create(recipe=self.recipe, portion=self.portion, quantity=2)

    def test_batch_update_empty_steps(self):
        """Test replacing all steps with empty list."""
        payload = {"recipe_slug": self.recipe.slug, "steps": []}
