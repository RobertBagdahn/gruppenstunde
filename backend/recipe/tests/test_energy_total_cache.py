...
class TestRecipeEnergyTotalCache:
    def test_recalculate_recipe_cache_sets_total_energy_from_weight(self):
        ...
        expected_total = ingredient.energy_kj * (item.quantity * item.portion.weight_g / 100.0)
        assert recipe.cached_energy_kj == pytest.approx(500.0)
        assert recipe.cached_energy_total_kj == pytest.approx(expected_total)
    ...
    def test_recipe_item_save_updates_total_energy_cache(self):
        ...
        recipe.refresh_from_db()
        assert recipe.cached_energy_total_kj == pytest.approx(500.0)

        item.quantity = 2.0
        item.save()

        recipe.refresh_from_db()
        assert recipe.cached_energy_total_kj == pytest.approx(1000.0)
    ...
    def test_recipe_item_delete_clears_total_energy_cache(self):
        ...
        recipe.refresh_from_db()
        assert recipe.cached_energy_total_kj is None
