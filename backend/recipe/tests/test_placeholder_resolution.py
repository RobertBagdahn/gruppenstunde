...
class TestPlaceholderResolution(TestCase):
    ...
    def setUp(self):
        ...
        self.flour_portion = Portion.objects.create(ingredient=self.flour, measuring_unit=self.unit, quantity=100)
        self.water_portion = Portion.objects.create(ingredient=self.water, measuring_unit=self.unit, quantity=250)
        self.salt_portion = Portion.objects.create(ingredient=self.salt, measuring_unit=self.unit, quantity=5)

        self.flour_item = RecipeItem.objects.create(recipe=self.recipe, portion=self.flour_portion, quantity=2, id=1)
        self.water_item = RecipeItem.objects.create(recipe=self.recipe, portion=self.water_portion, quantity=1, id=2)
        self.salt_item = RecipeItem.objects.create(recipe=self.recipe, portion=self.salt_portion, quantity=0.5, id=3)

        # Create a step
        self.step = RecipeStep.objects.create(
            recipe=self.recipe, instruction="Mix {Flour} with {Water} and a pinch of {Salt}", sort_order=1
        )
    ...
    def test_resolve_placeholder_by_ingredient_name(self):
        ...
        recipe_item_map = {item.id: item for item in self.recipe.recipe_items.all()}
    ...
    def test_resolve_placeholder_by_recipe_item_id(self):
        ...
        step = RecipeStep.objects.create(recipe=self.recipe, instruction="Mix {1} with {2} and {3}", sort_order=2)
        recipe_item_map = {item.id: item for item in self.recipe.recipe_items.all()}
    ...
    def test_resolve_multiple_placeholders(self):
        ...
        step = RecipeStep.objects.create(
            recipe=self.recipe, instruction="Add {Flour}, then {Water}, finally {Salt}", sort_order=2
        )
        recipe_item_map = {item.id: item for item in self.recipe.recipe_items.all()}
    ...
    def test_resolve_mixed_placeholder_formats(self):
        ...
        step = RecipeStep.objects.create(recipe=self.recipe, instruction="Mix {Flour}, {2}, and {Salt}", sort_order=2)
        recipe_item_map = {item.id: item for item in self.recipe.recipe_items.all()}
    ...
    def test_resolve_placeholder_case_insensitive(self):
        ...
        step = RecipeStep.objects.create(recipe=self.recipe, instruction="Mix {flour} and {water}", sort_order=2)
        recipe_item_map = {item.id: item for item in self.recipe.recipe_items.all()}
    ...
    def test_resolve_step_with_no_placeholders(self):
        ...
        step = RecipeStep.objects.create(recipe=self.recipe, instruction="Mix everything together", sort_order=2)
        recipe_item_map = {item.id: item for item in self.recipe.recipe_items.all()}
    ...
    def test_resolve_empty_instruction(self):
        ...
        step = RecipeStep.objects.create(recipe=self.recipe, instruction="", sort_order=2)
        recipe_item_map = {item.id: item for item in self.recipe.recipe_items.all()}
    ...
    def test_resolve_placeholder_with_special_chars(self):
        ...
        step = RecipeStep.objects.create(
            recipe=self.recipe, instruction="Mix {Flour} (100g), {Water} (250ml). Don't add {Salt}!", sort_order=2
        )
        recipe_item_map = {item.id: item for item in self.recipe.recipe_items.all()}
