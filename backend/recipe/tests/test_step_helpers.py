...
from recipe.models import RecipeItem, RecipeStep, RecipeStepIngredient
from recipe.services.step_helpers import generate_description_from_steps, resolve_placeholders
...
class TestResolvePlaceholders:
    ...
    def setup(self, db):
        ...
        gram_unit, _ = MeasuringUnit.objects.get_or_create(name="Gramm", defaults={"quantity": 1.0, "unit": "g"})
        liter_unit, _ = MeasuringUnit.objects.get_or_create(name="Liter", defaults={"quantity": 1.0, "unit": "ml"})
        piece_unit, _ = MeasuringUnit.objects.get_or_create(name="Stück", defaults={"quantity": 1.0, "unit": "stk"})
        ...
        # Create portions
        flour_portion = Portion.objects.create(ingredient=flour, measuring_unit=gram_unit, quantity=1.0, weight_g=1.0)
        sugar_portion = Portion.objects.create(ingredient=sugar, measuring_unit=gram_unit, quantity=1.0, weight_g=1.0)
        onion_portion = Portion.objects.create(
            ingredient=onion, measuring_unit=piece_unit, quantity=1.0, weight_g=120.0
        )
        milk_portion = Portion.objects.create(ingredient=milk, measuring_unit=liter_unit, quantity=1.0, weight_g=1000.0)

        # Create recipe items
        flour_item = RecipeItem.objects.create(recipe=recipe, portion=flour_portion, quantity=500, sort_order=1)
        sugar_item = RecipeItem.objects.create(recipe=recipe, portion=sugar_portion, quantity=200, sort_order=2)
        onion_item = RecipeItem.objects.create(recipe=recipe, portion=onion_portion, quantity=2, sort_order=3)
        milk_item = RecipeItem.objects.create(
            recipe=recipe, portion=milk_portion, quantity=Decimal("0.5"), sort_order=4
        )

        # Create step with recipe items
        step = RecipeStep.objects.create(recipe=recipe, instruction="Test instruction", sort_order=1)
        RecipeStepIngredient.objects.create(step=step, recipe_item=flour_item, sort_order=1)
        RecipeStepIngredient.objects.create(step=step, recipe_item=sugar_item, sort_order=2)
        RecipeStepIngredient.objects.create(step=step, recipe_item=onion_item, sort_order=3)
        RecipeStepIngredient.objects.create(step=step, recipe_item=milk_item, sort_order=4)
...
class TestGenerateDescriptionFromSteps:
    ...
    def recipe_with_steps(self, db):
        ...
        butter_portion = Portion.objects.create(ingredient=butter, measuring_unit=gram_unit, quantity=1.0, weight_g=1.0)
        eggs_portion = Portion.objects.create(ingredient=eggs, measuring_unit=gram_unit, quantity=1.0, weight_g=50.0)
        flour_portion = Portion.objects.create(ingredient=flour, measuring_unit=gram_unit, quantity=1.0, weight_g=1.0)

        butter_item = RecipeItem.objects.create(recipe=recipe, portion=butter_portion, quantity=200, sort_order=1)
        eggs_item = RecipeItem.objects.create(recipe=recipe, portion=eggs_portion, quantity=3, sort_order=2)
        flour_item = RecipeItem.objects.create(recipe=recipe, portion=flour_portion, quantity=300, sort_order=3)
        ...
        RecipeStepIngredient.objects.create(step=step1, recipe_item=butter_item, sort_order=1)
        ...
        RecipeStepIngredient.objects.create(step=step2, recipe_item=eggs_item, sort_order=1)
        RecipeStepIngredient.objects.create(step=step2, recipe_item=flour_item, sort_order=2)
