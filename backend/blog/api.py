...
@router.delete("/{blog_id}/image/", response=dict)
def delete_blog_image(request, blog_id: int):
    """Remove the title image from a blog."""
    blog = get_object_or_404(Blog, id=blog_id)
    can_edit = request.user.is_authenticated and (
        request.user.is_staff or blog.authors.filter(id=request.user.id).exists()
    )
...
@router.post("/{blog_id}/image-from-url/", response=dict)
def set_blog_image_from_url(request, blog_id: int, payload: ImageFromUrlIn):
