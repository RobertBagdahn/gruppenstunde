...
1. Basic info (name, description, retail section)
2. Physical properties (density, viscosity, durability, storage temp)
3. Allergens/Nutritional tags
4. Scores (child, scout, environmental, NOVA)
5. Recipe info (standard weight, raw consumption)
6. Nutritional values per 100g

Each step has its own Pydantic schema for structured output.
Spec reference: openspec/specs/ai-features/spec.md, lines 149-215
"""
...
class IngredientAIService:
    ...
    def _build_prompt(self, step: str, ingredient: "Ingredient") -> str:
        ...
        if step == "tags":
            # Provide available tags
            from idea.models import NutritionalTag
        ...
        return base  # Fallback (should not happen)
