...
def generate_instagram_slides(idea) -> list[str]:
    """
    Generate 3 Instagram slides for an idea.
    ...
    # If idea has an image, use it as background with overlay
    if idea.image:
        try:
            bg_img = Image.open(idea.image.path)
    ...
    desc = idea.summary_long or idea.summary or idea.description
    ...
    # Collect meta info
    from idea.choices import CostsRatingChoices, DifficultyChoices, ExecutionTimeChoices

    meta_lines = [
        f"⏱ Dauer: {ExecutionTimeChoices(idea.execution_time).label}",
        f"📊 Schwierigkeit: {DifficultyChoices(idea.difficulty).label}",
        f"💰 Kosten: {CostsRatingChoices(idea.costs_rating).label}",
    ]

    # Scout levels
    levels = [sl.name for sl in idea.scout_levels.all()]
    if levels:
        meta_lines.append(f"🏕 Stufen: {', '.join(levels)}")

    # Tags
    tags = [t.name for t in idea.tags.all()[:8]]
    if tags:
