...
def create_ingredient_copy(ingredient: dict, new_name: str, new_pk: int) -> dict:
    ...
    new_entry = {"model": ingredient["model"], "pk": new_pk, "fields": {**ingredient["fields"]}}

    # Update name and slug
    new_entry["fields"]["name"] = new_name
    new_entry["fields"]["slug"] = (
        new_name.lower().replace(" ", "-").replace("ä", "ae").replace("ö", "oe").replace("ü", "ue").replace("ß", "ss")
    )
