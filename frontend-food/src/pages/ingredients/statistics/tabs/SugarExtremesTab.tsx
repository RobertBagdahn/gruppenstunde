...
export default function SugarExtremesTab() {
  const [searchParams] = useSearchParams();
  const retailSectionId = searchParams.get('retail_section') ? Number(searchParams.get('retail_section')) : null;
  const { data, isLoading } = useIngredientRankings('sugar_g', { retailSectionId });

  return (
    <div className="space-y-4">
      <p className="text-sm text-muted-foreground">
        Die zuckerreichsten und zuckerärmsten verifizierten Zutaten pro 100g.
      </p>
