...
export default function GlutenRadarTab() {
  const [searchParams] = useSearchParams();
  const retailSectionId = searchParams.get('retail_section') ? Number(searchParams.get('retail_section')) : null;
  const { data, isLoading } = useIngredientTagLists('gluten', { retailSectionId });

  return (
    <div className="space-y-4">
      <p className="text-sm text-muted-foreground">
        Alle Zutaten, die mit dem Tag „Gluten" markiert sind – nützlich für die Lagerküche bei Zöliakie.
      </p>
      ...
      {isLoading ? (
        ...
        <div className="space-y-2">
          <p className="text-xs text-muted-foreground">
            {data.total_count} von {data.total_overall} verifizierten Zutaten enthalten Gluten
          </p>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-border text-muted-foreground">
                  ...
                  <th className="text-right py-2 px-3 font-medium hidden sm:table-cell">Protein</th>
                  <th className="text-right py-2 px-3 font-medium hidden md:table-cell">Energie</th>
              ...
              <tbody>
                {data.items.map((item) => (
                  <tr key={item.id} className="border-b border-border/30 hover:bg-muted/20 transition-colors">
                    ...
                    <td className="py-2 px-3 text-right font-mono text-xs hidden sm:table-cell">{item.protein_g ?? '–'}</td>
