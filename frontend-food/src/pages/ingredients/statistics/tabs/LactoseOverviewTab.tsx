...
export default function LactoseOverviewTab() {
  const [searchParams] = useSearchParams();
  const retailSectionId = searchParams.get('retail_section') ? Number(searchParams.get('retail_section')) : null;
  const { data, isLoading } = useIngredientTagLists('lactose', { sortBy: 'name', retailSectionId });
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-border text-muted-foreground">
                  ...
                  <th className="text-right py-2 px-3 font-medium">Laktose</th>
                  <th className="text-right py-2 px-3 font-medium hidden sm:table-cell">Fett</th>
