...
interface AllergenScanViewProps {
  mealPlanId: number;
  canEdit: boolean;
  onOpenSettings?: () => void;
...
export default function AllergenScanView({
  mealPlanId,
  canEdit,
  onOpenSettings,
  nutritionalTagsCount,
}: AllergenScanViewProps) {
  const { data, error, isLoading, refetch } = useAllergenScan(mealPlanId);
  const [collapsedAllergens, setCollapsedAllergens] = useState<Record<number, boolean>>({});
  ...
  if (violations.length === 0) {
    return (
      <div className="rounded-xl border border-primary/20 bg-emerald-500/5 p-6 text-center space-y-3 shadow-soft">
        ...
        <div className="space-y-1">
          <h3 className="font-display font-bold text-lg text-emerald-800">Ernährungs-Check: Keine Verstöße!</h3>
  ...
  const toggleAllergenCollapse = (tagId: number) => {
    setCollapsedAllergens((prev) => ({
      ...prev,
      [tagId]: !prev[tagId],
    }));
  };
      ...
      <div className="space-y-3">
        ...
        <div className="space-y-3">
          {Object.entries(groupedViolations).map(([tagIdStr, group]) => {
            ...
            const isCollapsed = collapsedAllergens[tagId] || false;
            ...
            return (
              ...
              >
                ...
                <button
                  type="button"
                  onClick={() => toggleAllergenCollapse(tagId)}
